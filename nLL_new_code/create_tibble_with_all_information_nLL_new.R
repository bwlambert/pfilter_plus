...
# find the best model based on the lowest nLL (nll_new)
best_mod_new <- df_cfr %>% group_by(country, model) %>%  # group by country and model
    summarise(nll_new = min(nll_new)) %>%     # get the corresponding nll
    group_by(country) %>%                               # then group result by country
    summarize(nll_new = min(nll_new))         # and choose the model with the lowest nll

best_mod_orig <- df_cfr %>% group_by(country, model) %>%  # group by country and model
...
best_mod_new$best <- "best_new"  # label model with "best"
best_mod_orig$best <- "best_orig"

# join with original datset so "best" labels appears on the model with lowest nll_orig
df_cfr_best_new <- full_join(df_cfr,best_mod_new,by=c("country","nll_new")) 
df_cfr_best_orig <- full_join(df_cfr,best_mod_orig,by=c("country","nll_orig")) 

# dfn <- df_cfr_best_new %>% select(country, model, nll_new, best)
dfn <- df_cfr_best_new %>% filter(best=="best_new")
# dfn <- dfn %>% distinct(country, model)
# names(dfn) <- c("country", "best_model_nll_new")

# dfo <- df_cfr_best_orig %>% select(country, model, nll_orig, best)
dfo <- df_cfr_best_orig %>% filter(best=="best_orig")
# dfo <- dfo %>% distinct(country, model)
# names(dfo) <- c("country", "best_model_nll_orig")

## FIX WHO (NON-MODEL) COUNTRIES
# remove WHO countries and then put them back into the df
who_iso <- read.csv(file="~/Desktop/input-ignore.indices.csv")
who_reported <- read.csv(file="~/Desktop/input-cases.csv",row.names = 1)

non_who_countries_dfo <- all_dat %>%        # subset out all models that are non-who method countries
  filter(is.na(match(country,who_iso$iso))) # filter to all models and non-who method countries

non_who_countries_dfn <- all_dat_new %>%        # subset out all models that are non-who method countries
  filter(is.na(match(country,who_iso$iso))) # filter to all models and non-who method countries
...
all_country_model_new <- rbind(non_who_countries_dfn[,-1], 
            data.frame(year = rep(1981:2020,length(who_iso$iso)), age = NA,
...
setwd("~/Desktop")
write.csv(all_country_model_new, "all_country_model_tibble_new.csv")
# write.csv(all_country_model_orig, "all_country_model_tibble_orig.csv")
...
df_summary <- all_country_model %>% filter(country == isoIdx[1]) %>% # subset to country == iso
  ...
  summarise(nll = min(nll_new)) %>%    # get the nll per model
